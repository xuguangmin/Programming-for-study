...
class SimpleGrid(grid.Grid):
	def __init__(self, parent):
		...
		self.SetColLabelValue(0, u'第1列')#设置列标题
		self.SetColLabelAlignment(wx.ALIGN_LEFT, wx.ALIGN_BOTTOM)
		self.SetRowLabelValue(0, u'第１行') #设置行标题
		self.SetRowLabelAlignment(wx.ALIGN_RIGHT, wx.ALIGN_BOTTOM)
		self.SetCellValue(0, 0, "Hello")
		self.SetCellTextColour(0, 0, wx.RED) #设置单元格字体
		...
		self.SetCellBackgroundColour(1, 0, wx.RED)#设置单元格背景颜色
		...
		self.SetCellAlignment(5, 1, wx.ALIGN_CENTER, wx.ALIGN_BOTTOM)
		...
		self.SetCellEditor(7, 0, grid.GridCellBoolEditor())#嵌入复选列表
		self.SetCellValue(7, 0, "True")

class MyFrame(wx.Frame):
	def __init__(self):
		wx.Frame.__init__(self, None, -1, u'单元格的设置', size = (650, 300))
