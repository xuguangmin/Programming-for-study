...
class MyGrid(grid.Grid):
	def __init__(self, parent):
		...
		self.CreateGrid(5, 5)
		#表格的事件
		self.Bind(grid.EVT_GRID_CELL_LEFT_CLICK, self.OnCellLeftClick)#当鼠标左键单击时触发
		self.Bind(grid.EVT_GRID_CELL_RIGHT_CLICK, self.OnCellRightClick)#当鼠标右键单击时触发
		...
		self.Bind(grid.EVT_GRID_CELL_CHANGE, self.OnCellChange)#当单元格内容改变时触发
		#当编辑单元格时触发
...
class MyFrame(wx.Frame):
	def __init__(self):
		wx.Frame.__init__(self, None, -1, u'表格的事件', size = (600, 350))
